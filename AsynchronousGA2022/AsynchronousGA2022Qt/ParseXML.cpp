...
ParseXML::XMLElement *ParseXML::GetElement(size_t index)
{
    return m_elementList[index].get();
}

size_t ParseXML::GetElementCount()
{
...
std::string ParseXML::lastError() const
{
    return m_lastError;
}

std::string *ParseXML::lastErrorPtr()
{
    return &m_lastError;
}

void ParseXML::setLastError(const std::string &lastError)
{
...
rapidxml::xml_attribute<char> *ParseXML::FindXMLAttribute(rapidxml::xml_node<char> *cur, const std::string &name, bool caseSensitive)
{
    ...
    setLastError("Attribute \""s + name + "\" not found"s);
    return ptr;
